...
The EntityCollection is a class that manages the list of entities. They have a private `final List<Entity> _entities` field.
...
* `List<Entity> toList() => _entities.toList()` returns copy of the list of entities in the collection. This is important to prevent external code from modifying the collection's list of entities.
* `Iterable<Entity> toIterator() => _entities;` returns an iterator of the entities in the collection.
